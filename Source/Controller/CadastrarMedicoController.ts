...
class CadastrarMedicoController {
    async tratar(request: Request, response: Response) {
        const { nome, crm, id_especialidade, endereco, telefone, email, senha } = request.body;
        const cadastrarMedicoService = new CadastrarMedicoService();
        const medico = await cadastrarMedicoService.executar({ nome, crm, id_especialidade, endereco, telefone, email, senha });
